...
const EXPECTED_MENU_ITEMS: Record<TestRole, string[]> = {
  ...
  operator: ['台帳問い合わせ', '区画残数管理', '書類管理'],
  manager: ['台帳問い合わせ', '合祀管理', '区画残数管理', '書類管理', 'ゆうちょ連携', 'スタッフ管理'],
  admin: ['台帳問い合わせ', '合祀管理', '区画残数管理', '書類管理', 'ゆうちょ連携', 'スタッフ管理', 'マスタ管理'],
};

// 各ロールで見えてはいけないメニュー項目
const HIDDEN_MENU_ITEMS: Record<TestRole, string[]> = {
  viewer: ['合祀管理', 'ゆうちょ連携', 'スタッフ管理', 'マスタ管理'],
