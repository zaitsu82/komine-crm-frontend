...
const EXPECTED_MENU_ITEMS: Record<TestRole, string[]> = {
  ...
  manager: ['台帳問い合わせ', '合祀管理', '区画残数管理', '書類管理', 'スタッフ管理', '一括登録・編集'],
  admin: ['台帳問い合わせ', '合祀管理', '区画残数管理', '書類管理', 'スタッフ管理', 'マスタ管理', '一括登録・編集'],
};

// 各ロールで見えてはいけないメニュー項目
const HIDDEN_MENU_ITEMS: Record<TestRole, string[]> = {
  viewer: ['スタッフ管理', 'マスタ管理', '一括登録・編集'],
  operator: ['スタッフ管理', 'マスタ管理', '一括登録・編集'],
  manager: ['マスタ管理'],
  admin: [],
};
